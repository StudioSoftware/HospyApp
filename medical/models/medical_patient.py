...
class MedicalPatient(models.Model):
    ...
    is_whatapps = fields.Boolean(
        string="mobile  is Whatapps",
        default=False)
    ...
    def _create_vals(self, vals):
        ...
        title=self.env['res.partner.title'].search_read(
            [('id','=',vals['title'])],fields=['name']
            ) 
        msg='Bonjour !' + title[0]['name'] + vals['name']+'Vous etes enregistre dans notre Clinique Coeur et Vie votre numero ID patient est:'+ vals['identification_code']
        if vals['mobile']:
            url = 'https://www.agis-as.com/epolice/index.php?telephone=' + temp + '&message='+msg
            requests.request('GET', url)
        if vals['is_whatapps']==True:
            url = 'https://api.whatsapp.com/send?phone=' + temp + '&message='+msg
            requests.request('GET', url)
        # return super(OeMedicalPatient, self).create(vals)
        return vals
